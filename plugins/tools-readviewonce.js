...
let handler = async (m, { conn }) => {
    if (!m.quoted) return conn.reply(m.chat, `🥺 ¡Oh no! Necesito que respondas a una imagen ViewOnce para poder ayudarte...`, m)
    if (!m?.quoted || !m?.quoted?.viewOnce) return conn.reply(m.chat, `😊 ¡Hola! Responde a una imagen ViewOnce para continuar...`, m)
